package com.ahuo.tools.network.retrofit;

import android.content.Context;

import com.ahuo.tools.network.config.AuthenticatorManager;
import com.ahuo.tools.network.config.CommonInterceptor;
import com.ahuo.tools.network.config.CookieManager;
import com.ahuo.tools.network.config.FastJsonConverterFactory;
import com.ahuo.tools.network.config.LoggingInterceptor;
import com.ahuo.tools.network.download.DownLoadService;
...
public class KKNetWorkRequest {
    ...
    public <T extends BaseResponseEntity> void asyncNetWork(final String TAG, final int requestCode, final Call<T> requestCall, final KKNetworkResponse<T> responseListener) {
        ...
        call.enqueue(new Callback<T>() {
            ...
            public void onFailure(Call<T> call, Throwable t) {
                cancelCall(TAG, requestCode);
                responseListener.onDataError(requestCode, 0, NetErrCodeConfig.getErrString(mContext, t));
            }
        });
    }
    ...
    public <T extends BaseResponseEntity> void syncNetWork(final String TAG, final int requestCode, final Call<T> requestCall, final KKNetworkResponse<T> responseListener) {
        ...
        try {
            ...
            responseListener.onDataError(requestCode, 0, NetErrCodeConfig.getErrString(mContext, e));
        } finally {
            cancelCall(TAG, requestCode);
        }
    }
